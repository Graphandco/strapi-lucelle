   ...
   return (
      <AnimatePresence mode="wait">
         <motion.li
            ...
            className="relative flex items-center justify-between py-2 px-1 cursor-pointer border-b border-white/10"
            onClick={async () => {
               try {
         ...
         >
            {pageType === "inventaire" ? (
               <motion.div
                  aria-hidden
                  className="pointer-events-none absolute inset-x-0 top-1/2 z-3 h-px -translate-y-1/2 bg-primary"
                  initial={false}
                  animate={{ scaleX: isToBuy ? 1 : 0 }}
                  transition={{
                     duration: 0.4,
                     ease: [0.22, 1, 0.36, 1],
                  }}
